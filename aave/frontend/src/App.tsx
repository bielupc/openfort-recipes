...
function App() {
  ...
  // Read USDC balance with refresh trigger
  ...
  // Pool and wallet balance refresh
  const refreshAllData = useCallback(async () => {
    await refetchUsdcBalance();
    forceRefresh();
  }, [refetchUsdcBalance, forceRefresh]);

  useEffect(() => {
    if (!isConnected || !address) return;
    const interval = setInterval(() => {
      if (!isSupplying && !isWithdrawing && !supplying.loading && !withdrawing.loading && !sending.loading) {
        refreshAllData();
      }
    }, 10000);
    return () => clearInterval(interval);
  }, [isConnected, address, isSupplying, isWithdrawing, supplying.loading, withdrawing.loading, sending.loading, refreshAllData]);
